...
<div class="container">
    <div style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); margin-bottom: 40px; display: flex; gap: 15px; flex-wrap: wrap; align-items: center;">
        ...
        <select id="filterNivel" style="flex: 1; padding: 12px; border: 1px solid #ddd; border-radius: 8px; outline: none; background: white;">
            <option value="todos">Todos los Niveles</option>
            <option value="Ingeniería">Ingeniería</option>
            <option value="Maestría">Maestría</option>
            <option value="Doctorado">Doctorado</option>
        </select>
        <select id="filterAnio" style="flex: 1; padding: 12px; border: 1px solid #ddd; border-radius: 8px; outline: none; background: white;">
    ...
    <div class="grid" id="catalogoGrid">
        ...
        <div class="card proyecto-item" data-nivel="{{ proyecto.nivel }}" data-anio="{{ proyecto.anio }}" style="display: flex; flex-direction: column; justify-content: space-between; min-height: 250px; padding: 25px;">
            <div>
                <span style="display: inline-block; font-size: 0.75rem; color: var(--azul-primario); font-weight: bold; background: rgba(13, 110, 253, 0.1); padding: 4px 12px; border-radius: 20px; margin-bottom: 15px;">
                    {{ proyecto.anio }} | {{ proyecto.nivel }}
                </span>
                <h3 class="proyecto-titulo" style="margin: 0 0 10px 0; color: var(--azul-oscuro); font-size: 1.25rem; line-height: 1.4;">
                    {{ proyecto.titulo }}
                </h3>
                <p class="proyecto-autor" style="font-size: 0.9rem; color: #555; margin-bottom: 15px;">
                    <i class="fas fa-user-edit" style="margin-right: 5px; color: #888;"></i> <strong>Por:</strong> {{ proyecto.autor }}
                </p>
                <p style="color: #666; font-size: 0.95rem; line-height: 1.5; margin: 0;">
                    {{ proyecto.resumen | truncate: 140 }}
                </p>
            </div>
            
            <div style="margin-top: 20px; border-top: 1px solid #eee; padding-top: 15px;">
                <a href="{{ proyecto.link }}" style="color: var(--azul-primario); font-weight: bold; text-decoration: none; display: flex; align-items: center; gap: 5px;">
                    Ver más <i class="fas fa-arrow-right" style="font-size: 0.8rem;"></i>
                </a>
            </div>
        </div>
        {% endfor %}
...
<script>
    ...
    function filtrarProyectos() {
        ...
        items.forEach(item => {
            ...
            // IMPORTANTE: Aquí corregimos el display para que no se rompa el diseño
